...
| D-013 | 2026-09-08 | accepted | Add picker search, horizontal category shortcuts, and expandable category sections with one section shown at a time by default. | A 47-item catalogue cannot rely on vertical scanning alone. These controls make exact lookup and category browsing fast while keeping exercise selection in one sheet. | Category changes use restrained native-style motion and respect Reduce Motion. Search shows every matching group with static headings; browse-mode headers remain labelled buttons with expanded state. |

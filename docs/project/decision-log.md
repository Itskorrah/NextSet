...
| D-011 | 2026-09-08 | accepted | Use the owner-selected **NS Monogram** as the NextSet application icon: black N, vermilion S, warm paper field. | The owner rejected the generated variation and selected the exact fourth mark from the supplied concept sheet for its clear, compact identity. | The exact artwork is packaged at `mobile/assets/nextset-icon.png` and configured for native builds. Re-evaluate after real home-screen testing or before public brand release. |
| D-012 | 2026-09-08 | accepted | Expand the built-in exercise catalogue to 47 mainstream movements and group the picker as Chest & shoulders, Back, Legs, Arms, and Bodyweight & core. | The initial eight-item list was too barbell-heavy and difficult to scan. Grouped built-ins cover common machine, cable, free-weight and bodyweight logging while retaining custom exercise entry. | Each movement has a stable key and an existing supported measurement mode. Assisted movements whose load semantics are not yet modelled remain custom-only. Search is deferred until real use shows that category browsing is insufficient. |
| D-013 | 2026-09-08 | accepted | Add picker search, persistent horizontal category shortcuts, and independently expandable category sections that are all open by default. | A 47-item catalogue cannot rely on vertical scanning alone. These controls make exact lookup and category browsing fast while keeping exercise selection in one sheet. | The search-and-shortcut area stays fixed while results scroll below it. Category jumps never close a section; browse-mode headers remain labelled buttons with expanded state. Motion is restrained and respects Reduce Motion. |
